...
# ─────────────────────────────────────────────────────────────────────────────
# 2. LLM tool BOLA — agent tools must authorize against the verified session
# ─────────────────────────────────────────────────────────────────────────────
...
def _seed_session(session_id="s", *, cid=""):
    import conversation_store as cs
...
def test_session_verified_cid_reads_top_level_only(monkeypatch):
    import agent
    import conversation_store as cs
    # Top-level (SDK-set) is trusted; a tool-set customer_info copy is not.
    cs._conversations.set({"s": {"salla_customer_id": "555",
                                 "customer_info": {"salla_customer_id": "999"}}})
    assert agent._session_verified_cid("s") == "555"
    cs._conversations.set({"s": {"customer_info": {"salla_customer_id": "999"}}})
    assert agent._session_verified_cid("s") == ""        # not verified


# ── track_order ───────────────────────────────────────────────────────────────

async def test_track_order_unverified_refuses_without_query(monkeypatch):
    import agent
    import conversation_store as cs
    monkeypatch.setattr(cs, "restore_to_memory", _anoop)
    salla = _FakeSalla(order={"id": 1, "customer": {"id": "999"}})
    a = _bare_agent(salla)
    _seed_session(cid="")                                # not identified
...
async def test_track_order_owned_succeeds(monkeypatch):
    ...
    a = _bare_agent(salla)
    _seed_session(cid="555")
    out = await a._run_tool("track_order", {"order_reference": "10234"}, "s")
    assert "10234" in out


async def test_track_order_other_customer_refused(monkeypatch):
    """Verified as 555 but order belongs to 999 → must not disclose."""
    import conversation_store as cs
    monkeypatch.setattr(cs, "restore_to_memory", _anoop)
...
async def test_lookup_customer_unverified_refuses_no_phone_lookup(monkeypatch):
    import agent
    import conversation_store as cs
    monkeypatch.setattr(cs, "restore_to_memory", _anoop)
...
async def test_lookup_customer_verified_returns_only_own(monkeypatch):
    ...
    assert "Sara" in out
    assert ("get_customer", 555) in salla.calls          # looked up the SESSION id
    assert not any(c[0] == "get_customer_by_phone" for c in salla.calls)


# ── get_order_invoice ─────────────────────────────────────────────────────────

async def test_invoice_unverified_refuses(monkeypatch):
    import agent
    import conversation_store as cs
    monkeypatch.setattr(cs, "restore_to_memory", _anoop)
...
async def test_invoice_other_customer_refused(monkeypatch):
    import conversation_store as cs
    monkeypatch.setattr(cs, "restore_to_memory", _anoop)
...
async def test_abandoned_carts_unverified_refuses(monkeypatch):
    import agent
    import conversation_store as cs
    monkeypatch.setattr(cs, "restore_to_memory", _anoop)
...
async def test_abandoned_carts_only_own(monkeypatch):
    import conversation_store as cs
    monkeypatch.setattr(cs, "restore_to_memory", _anoop)
