...
الأرقام الداخلية التالية **سرية**، يحسبها البوت داخلياً لكن **لا يذكرها للعميل أبداً** تحت أي ظرف:
  ...
  ✗ المعادلات والشرائح والـ tiers الداخلية
...
class PrintingAgent:
    ...
    async def _chat_anthropic(self, message: str, session_id: str) -> str:
        ...
        # ── Prompt caching (ephemeral, 5-min TTL) ─────────────────────────────
        # System prompt (~3-4k tokens) and tool definitions (~2k tokens) are
        # identical across every request for the same store, so caching them
        # cuts input billing by ~90% on the cached portion. Two breakpoints:
        #   1) end of system  → caches the system block
        #   2) last tool      → caches system + all tools together
        # Cache hits also reduce latency (cached tokens skip re-processing).
        # Build once per turn so every tool-use round in the loop hits cache.
        system_text = await get_system_prompt_async(self.store_id)
        cached_system = [{
            "type":          "text",
            "text":          system_text,
